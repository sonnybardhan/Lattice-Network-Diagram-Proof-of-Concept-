...
Create a proof of concept web application to visualize 121 mental models using Cytoscape.js. This is a standalone project to test visualization approaches before integrating into a larger application called Lattice.

The goal is to implement four distinct visualization views and test the filtering logic that narrows 121 models down to actionable clusters of 5-6.
...
- No backend required - all data is static JSON/TypeScript files
...
## Complete Data: Tags with L1/L2 Questions

Create in `src/data/tags.ts`:
...
## Complete Data: All 121 Mental Models

Create in `src/data/models.ts`:
...
Create in `src/data/edges.ts`. Generate edges from the adjacentModels arrays in models.ts:
...
## Complete Data: Learning Paths

Create in `src/data/learningPaths.ts`:
...
### View 1: Filtered Subgraph (FilteredSubgraph.tsx)

**Purpose:** Display the 4-6 models resulting from L1/L2 filtering after a user describes a scenario.

**Layout:** Force-directed or cose layout (Cytoscape)

**Features:**

- All nodes visible with edges showing relationships
- Nodes sized uniformly, "best fit" node slightly larger or highlighted
- Click node → transition to EgoNetwork view for that model
- Side panel shows list of models with one-line insights
- Edge thickness based on strength (if data available)

**UI Components:**

- Graph container (left, ~60% width)
- Model list panel (right, ~40% width)
- Each model in list shows: name, diagnostic question, "Learn more →" link

---

### View 2: Ego-Network (EgoNetwork.tsx)

**Purpose:** Deep dive on one model, showing its immediate connections.

**Layout:** Concentric layout (Cytoscape) - focus node in center, adjacents in ring

**Features:**

- Center node is the focus model (larger, highlighted)
- Surrounding nodes are adjacent models (from adjacentModels array)
- Click adjacent node → it becomes the new center (animated transition)
- Side panel shows full model card with all details

**UI Components:**

- Graph container (left, ~50% width)
- Model card panel (right, ~50% width)
  - Name
  - Diagnostic question
  - Key insight
  - Red flag phrases (as chips/tags)
  - Adjacent models with "why adjacent" explanations
  - "Practice with scenario →" button (placeholder)

---

### View 3: Tag Cluster View (TagClusterView.tsx)

**Purpose:** Browse all 121 models organized by the 12 tags.

**Layout:** Grid of tag cards that expand to show models

**Collapsed State:**

- 12 cards in a grid (4x3 or responsive)
- Each card shows: tag name, model count, L1 question preview

**Expanded State:**

- Click tag → expands to show all models in that tag as nodes
- Use force-directed layout within the expanded view
- Edges show intra-tag relationships only
- Click model → transition to EgoNetwork view

**UI Components:**

- Grid container with tag cards
- Expanded view overlay/modal with graph
- Search/filter input to find specific model by name

---

### View 4: Learning Path (LearningPath.tsx)

**Purpose:** Guided sequential learning through curated model sequences.

**Layout:** Horizontal progress path + ego-network for current model

**Features:**

- Path selector dropdown to choose a learning path
- Horizontal progress indicator showing model sequence
- Completed models marked with checkmark
- Current model highlighted
- Ego-network view for current model
...
- Path selector (top)
- Progress bar with model nodes (top)
- Ego-network graph (left, ~50%)
- Model card (right, ~50%)
...
**Purpose:** Implement the L1/L2 filtering logic.

**L1 Stage:**

- Display 12 yes/no questions (or 6 in reduced mode)
- Questions in order specified (by concreteness)
- Toggle or checkbox for each
- "Apply filters" button

**L2 Stage:**

- For each "yes" answer with pool > 6, show L2 question
- Show only 2-3 L2 questions max (for largest contributing tags)
- Radio buttons for L2 options
- Show resulting model count after each selection

**Result:**

- Display final cluster count
- "View results" button → navigates to FilteredSubgraph view

---

## Hook: useFilterLogic.ts
...
export type LayoutType = "concentric" | "cose" | "grid" | "breadthfirst";

export function useGraphLayout(layoutType: LayoutType, focusNodeId?: string) {
...
## GraphContainer Component (GraphContainer.tsx)
...
## Utility: graphUtils.ts

```typescript
import { MentalModel, Edge } from "../types";
import { edges } from "../data/edges";
import { models } from "../data/models";
import cytoscape from "cytoscape";

// Convert models to Cytoscape elements
export function modelsToCytoscapeElements(
  ...
  const nodes: cytoscape.ElementDefinition[] = modelIds.map((id) => {
  ...
  const edgeElements: cytoscape.ElementDefinition[] = relevantEdges.map(
    (e, i) => ({
      data: {
        id: `edge-${i}`,
        source: e.source,
...
// Get ego network (model + adjacent models)
...
import { FilteredSubgraph } from './components/views/FilteredSubgraph';
import { EgoNetwork } from './components/views/EgoNetwork';
import { TagClusterView } from './components/views/TagClusterView';
...
```bash
npm create vite@latest mental-models-poc -- --template react-ts
cd mental-models-poc
```
...
5. Create all files as specified in the structure above.
...
1. **Tag Cluster View (default):** User sees 12 tag cards. Clicking a tag expands to show its models as a graph.

2. **Scenario Mode:** User clicks "Start Scenario" → Filter Panel appears → User answers L1 yes/no questions → If pool > 6, L2 questions appear → Results show in Filtered Subgraph view.

3. **Ego Network:** Clicking any model anywhere transitions to its ego-network view. Clicking adjacent models navigates between them.

4. **Learning Paths:** User selects a path and progresses through models sequentially, with ego-network + card for each.

---

## Success Criteria

- [ ] All 121 models display correctly
- [ ] Cytoscape renders graphs smoothly with appropriate layouts
- [ ] L1/L2 filtering correctly narrows to ≤6 models
- [ ] Navigation between views works seamlessly
- [ ] Clicking nodes triggers appropriate actions
- [ ] Model cards display all relevant information
